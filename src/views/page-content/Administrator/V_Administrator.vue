...
export default {
  ...
  watch: {
    ...
    page: {
			...
			handler(value) {
        if(value){
          this.prosesData(value, this.limit, this.searchData, this.kumpulSort)
        }
			}
		},
    limit: {
			...
			handler(value) {
        this.page = 1
				this.prosesData(1, value, this.searchData, this.kumpulSort)
			}
		},
  },
  mounted() {
    ...
    this.idLog = localStorage.getItem('idLogin')
		this.prosesData(this.page, this.limit, this.searchData, this.kumpulSort);
	},
	methods: {
    ...mapActions({
      fetchData: 'fetchData',
    ...
    prosesData(page, limit, keyword, sorting) {
      this.getAdministrator(qs.stringify({ page, limit, keyword, sorting }, { encode: false }));
    },
    postRecord(item, jenis, kondisi) {
      ...
      .then((res) => {
        this.prosesData(1, this.limit, this.searchData, this.kumpulSort)
