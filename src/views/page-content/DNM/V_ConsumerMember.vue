<template>
  <div>
    <h2 class="subheading grey--text text-decoration-underline">Data Consumer Member</h2>
      ...
      >
        ...
        <template #[`item.idMember`]="{ item }">
          ...
          <br><Button 
            ...
            nama-button="Detail"
            size-button="x-small"
            @proses="detailMember(item.raw.idMember)"
          />
        </template>
        <template #[`item.Refcode`]="{ item }">
          <input type="hidden" id="testing-code-on" :value="item.raw.Refcode">
          <span ref="myinputon" v-html="item.raw.Refcode ? item.raw.Refcode : '-'" /> <v-icon v-if="item.raw.Refcode" @click.stop.prevent="copyText(item.raw.Refcode, 'Kode Referal')" size="small" icon="mdi mdi-content-copy" color="#28a154" />
        </template>
    ...
    >
      <v-card color="background-dialog-card">
        <v-toolbar color="surface">
          <v-toolbar-title>Detail Member</v-toolbar-title>
          <v-spacer />
          <v-toolbar-items>
            <Button
              variant="plain"
              ...
              model-button="comfortable"
              size-button="large"
              @proses="() => { jsonData = ''; dialogDetail = false; }"
            />
          </v-toolbar-items>
        </v-toolbar>
...
export default {
  ...
  data: () => ({
    ...
    pageOptions: [],
      ...
      { title: "Kode Referal", key: "Refcode", sortable: false },
  ...
  watch: {
    ...
    page: {
			...
			handler(value) {
        this.DataConsumer = []
        this.CheckProses(value, this.limit, this.tanggal, 1, this.searchData)
			}
		},
    limit: {
			deep: true,
        ...
        this.DataConsumer = []
        this.page = 1
				this.CheckProses(this.page, value, this.tanggal, 1, this.searchData)
			}
		},
  },
  mounted() {
    if(!localStorage.getItem('user_token')) return this.$router.push({name: 'LogIn'});
    this.CheckProses(this.page, this.limit, this.tanggal, 1, this.searchData)
	},
	methods: {
    ...mapActions({
      getConsumer: 'user/getConsumer',
      getDetailMemberKNET: 'user/getDetailMemberKNET',
    }),
    ...
    detailMember(idMember) {   
      ...
      .then((res) => {
        this.jsonData = res.data.result.length ? res.data.result[0] : null
